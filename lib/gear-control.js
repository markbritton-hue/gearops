...
(function (global) {
  ...
  const gear = {
    ...
    //   gear.ptzMove(ip, 'leftup', 12); gear.ptzStop(ip); gear.ptzZoom(ip, 'in');
    //   gear.ptzPreset(ip, 'call', 1); gear.ptzHome(ip);
    ptzList:   ()               => call('/ptz/list'),
    ptzMove:   (ip, dir, speed) => call('/ptz/move', speed != null ? { ip, dir, speed } : { ip, dir }),
    ptzStop:   (ip)             => call('/ptz/stop', { ip }),
    ptzZoom:   (ip, dir)        => call('/ptz/zoom', { ip, dir }),
    ptzFocus:  (ip, dir)        => call('/ptz/focus', { ip, dir }),
    ptzHome:   (ip)             => call('/ptz/home', { ip }),
    ptzPreset: (ip, action, n)  => call('/ptz/preset', { ip, action, n }),
  };
